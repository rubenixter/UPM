...
for file_name in os.listdir(pdf_dir):
    ...
    words = re.findall(r'target=(\w+)',  str(fres), re.DOTALL)
    if words:
        # If matches are found, add them to the url_list
        urls.extend(words)

    if urls:
        # Write URLs to a text file in the "results" folder
        with open('./Figures/urls'+str(i)+'.txt', 'w') as file:
            for url in urls:
                file.write(url + '\n')
    i=i+1
            
nombres=['Fichero 1','Fichero 2']
plt.figure(figsize=(6,6), facecolor=None)
plt.title('bars')
